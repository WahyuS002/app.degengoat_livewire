    ...
    <div wire:ignore.self  class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" x-show="open">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                @if ($shuffle->total_winners_amount > $shuffle->participants->count())
                ...
                @endif
                    <div class="bg-white px-4 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="flex justify-center">
                            <div class="inline-block mt-2 px-2 py-1 rounded-full bg-gray-200">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <span class="text-sm">
                                        Total Participants <span class="font-bold ml-1">{{ $shuffle->participants->count() }}</span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div class="mt-8">
                            ...
                            @else
                            <div class="w-full bg-green-100 px-2 py-3 rounded-lg">
                                <div class="flex justify-center items-center space-x-7">
                                    <span>
                                    ...
                                    <span class="break-words text-center font-medium">
                                        Do you want to random {{ $shuffle->participants->count() }} participants, for {{ $shuffle->total_winners_amount }} total winners?
                                    </span>
                                </div>
                            </div>
                            @endif
